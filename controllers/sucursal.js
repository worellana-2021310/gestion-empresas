...
const asignarEmpresa = async (req = request, res = response) => {
    ...
    if (!existeSucursal) {
        return res.status(404).json({
            msg: '----- Error al encontrar surcursal -----'
        })
    }
    ...
    if (existe) return res.status(400).json({ msg: '----- Surcursal existente -----' })
